...
class PizzaDatabase:
    ...
    def get_customer(self, id):
        if (not self.exits(table='customer', col='id', str=id)): return None
    ...
    def get_order(self, id):
        if (not self.exits(table='order_info', col='id', str=id)): return None
    ...
    def get_deliveryman(self, id):
        if (not self.exits(table='deliveryman', col='id', str=id)): return None
    ...
    def get_order_time(self, order_id):
        if (not self.exits('order_info', 'id', order_id)):
    ...
    def place_order(self, customer_id, pizzas, side_dishes):
        self.__execute(f"INSERT INTO order_info(customer) VALUES ({customer_id});")
        self.__execute("SELECT last_insert_id();")
        order_id = self.cursor.fetchone()[0]

        for id in pizzas: self.__execute(f"INSERT INTO order_to_pizza(order_info, pizza) VALUES ({order_id}, {id});")
        for id in side_dishes: self.__execute(f"INSERT INTO order_to_side_dish(order_info, side_dish) VALUES ({order_id}, {id});")
        self.__execute(f"UPDATE order_info SET time = NOW() WHERE id = {order_id};")

        self.db.commit()
        return order_id

    def delete_order(self, order_id):
        self.__execute(f"DELETE FROM order_to_pizza WHERE order_info = {order_id};")
        self.__execute(f"DELETE FROM order_to_side_dish WHERE order_info = {order_id};")
        self.__execute(f"DELETE FROM order_info WHERE id = {order_id};")
        self.db.commit()
    
    def check_coupon(self, coupon_id):
        if (not self.exits('coupon', 'id', coupon_id)): return False
    ...
    def print_pizza(self, pizza_id):
        ...
        print("  - P" + str(pizza_id) + ": " + pizza["name"].title() + " " + self.is_vegan(pizza["name"]))
        price = 0
    ...
    def __execute(self, command):
        for line in command.replace("\n", " ").replace("\t", " ").split(";"):
            ...
            try:
                ...
                self.db.commit()
            except sql.Error as error: print("ERROR: " + str(error))
